...
def main
  ...
  CSV.foreach(file) do |row|
    ...
    if (row[1]!=nil) #should not be nil anyway for any host to be added, for better logging outputs
	  ...
	  puts "\n Host: " + row[1] 
          puts rpc("addHost", {"hostName" =>row[1], "displayedAs" =>URI.escape(row[2]), "agentId" => row[0], "hostGroupIds" => "#{groupid},#{lmgroupid}"})
        else
          puts "\n Host: " + row[1]
      ...
      if(row[2]!=nil) #if displayname is nil and there is no fullpath, just place it the lmsupport-import host group
        puts "\n Host: " + row[1]
        puts rpc("addHost", {"hostName" =>row[1], "displayedAs" =>URI.escape(row[2]), "agentId" => row[0], "hostGroupIds" => "#{lmgroupid}"})
